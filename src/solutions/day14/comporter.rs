...
enum ProgramStatement<'a> {
    Instruction(usize, u64),
    Mask(&'a str),
}

impl<'a> ProgramStatement<'a> {
    fn parse(line: &'a str) -> Result<ProgramStatement> {
        ...
        let mut parser = alt((
            map(mem_statement(), |(addr, value)| ProgramStatement::Instruction(addr, value)),
            map(mask_statement(), ProgramStatement::Mask),
        ));
        let (_, result) = parser(line)
            .finish()
            .map_err(|e| anyhow!("{}", e))?;
        Ok(result)
    }
